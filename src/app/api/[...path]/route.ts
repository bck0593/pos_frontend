...
async function forwardOnce(url: string, req: NextRequest) {
  ...
  return fetch(url, { method, headers: forwardHeaders(req), body, redirect: "manual", cache: "no-store" });
}
...
async function proxy(req: NextRequest, parts: string[] = []) {
  ...
  try {
    let res = await forwardOnce(candidates[0], req);
    if (res.status === 404) res = await forwardOnce(candidates[1], req);
    ...
    return new Response(JSON.stringify({ error: "PROXY_FETCH_FAILED", message: emsg(e) }), {
      status: 502, headers: { "content-type": "application/json" }
    });
  }
}
...
export async function HEAD(req: NextRequest, ctx: Ctx) { return proxy(req, ctx.params.path ?? []); }
export async function OPTIONS(req: NextRequest, ctx: Ctx) { return proxy(req, ctx.params.path ?? []); }
